...
export class EmailService {
  ...
  constructor() {
    this.emailTemplates = new Map([
      [EjsTemplates.signIn, { templateFile: "SIGN-IN.ejs", subject: "[Smoketrees Digital] Sign In" }],
      [EjsTemplates.signUp, { templateFile: "SIGN-UP.ejs", subject: "[Smoketrees Digital] Sign Up" }],
      [EjsTemplates.welcome, { templateFile: "WELCOME.ejs", subject: "[Smoketrees Digital] Welcome" }],
      [EjsTemplates.emailVerification, { templateFile: "EMAIL_VERIFICATION.ejs", subject: "[Smoketrees Digital] Email Verification" }],
      [EjsTemplates.forgotPasswordOtp, { templateFile: "FORGOT_PASSWORD_OTP.ejs", subject: "[Smoketrees Digital] Forgot Password OTP" }],
    ]);
  }
  ...
  async sendTemplateEmail(
    ...
    try {
      ...
      const sendEmailResult = await sendEmail(subject, html, { html: true }, recipient.to, recipient.cc, recipient.bcc, recipient.from, recipient.replyTo);

      const result = new Result(!!sendEmailResult.$response.error, ErrorCode.Success, "Sent email request", sendEmailResult.MessageId);
  ...
  async generateHtml(data: ejsRenderParamsType) {
    ...
    if (templateFile) {
      const template = fs.readFileSync(path.resolve(__dirname, "..", "..", "..", "EmailTemplates", templateFile)).toString("utf-8");
